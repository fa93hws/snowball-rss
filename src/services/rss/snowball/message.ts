...
export class Post {
   ...
   * link: string,
   */
  static fromRaw(raw: any, author: string): Result.T<Post> {
    if (raw == null || typeof raw !== 'object') {
      return Result.err(new Error(`invalid raw post, expect object, but got ${raw}`));
    }
    if (typeof raw.title !== 'string') {
      return Result.err(
        new Error(`invalid raw post, expect title to be string, but got ${raw.title}`),
      );
    }
    if (typeof raw.description !== 'string') {
      return Result.err(
        new Error(`invalid raw post, expect description to be string, but got ${raw.description}`),
      );
    }
    if (typeof raw.link !== 'string') {
      return Result.err(
        new Error(`invalid raw post, expect link to be string, but got ${raw.link}`),
      );
    }
...
export class Message {
   ...
   */
  static fromRaw(raw: any): Result.T<Message> {
    if (raw == null || typeof raw !== 'object') {
      return Result.err(new Error(`invalid raw message, expect object, but got ${raw}`));
    }
    const title = raw.title;
    const suffix = ' 的雪球全部动态';
    if (typeof title !== 'string') {
      return Result.err(
        new Error(`invalid raw message, expect title to be string, but got ${raw.title}`),
      );
    } else if (!title.includes(suffix)) {
      return Result.err(
        new Error(`invalid title, expect title to be "<author>${suffix}", but got ${raw.title}`),
      );
    }
    const author = title.slice(0, -suffix.length);
    if (!Array.isArray(raw.item)) {
      return Result.err(
        new Error(`invalid raw message, expect item to be an array, but got ${raw.item}`),
      );
    }
